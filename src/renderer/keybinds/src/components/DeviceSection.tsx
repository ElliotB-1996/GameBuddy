...
export default function DeviceSection({ profile, activeZone, className }: Props) {
  const [activeLayer, setActiveLayer] = useState<'default' | 'shift'>('default')
  ...
  return (
    <div className={`device-section ${className ?? ''}`}>
      ...
      <div className={gridClass}>
        {grid.flat().map((id, i) =>
          id
            ? <Btn key={id} id={id} button={layer[id]} activeZone={activeZone} />
