...
const api = {
  ...
  onNotesLoad: (cb: (data: AppData, loadError: string | null) => void) => {
    ipcRenderer.on("notes:load", (_event, data, loadError) =>
      cb(data, loadError),
    );
  },
...
export function ModeIndicator({ mode, onToggle }: Props) {
  return (
    <button
      ...
      title={
        mode === "view"
          ? "Click or use hotkey to enter edit mode"
          : "Click or use hotkey to enter view mode"
      }
      style={{
        background: "none",
...
const checklistNote: Note = {
  id: "n2",
  type: "checklist",
  content: [{ id: "i1", text: "Item 1", checked: false }],
  createdAt: "2026-01-01T00:00:00Z",
  updatedAt: "2026-01-01T00:00:00Z",
};
const noop = () => {};
  ...
  return (
    ...
    >
      ...
      {note.type === "text" ? (
        ...
        <ChecklistNote
          items={note.content as ChecklistItem[]}
          isEditMode={isEditMode}
          onChange={onUpdate}
        />
      )}
...
describe("SettingsPanel", () => {
  it("renders all three hotkey fields", () => {
    render(
      <SettingsPanel hotkeys={hotkeys} onSave={vi.fn()} onClose={vi.fn()} />,
    );
  ...
  it("calls onClose when Cancel is clicked", async () => {
    const onClose = vi.fn();
    render(
      <SettingsPanel hotkeys={hotkeys} onSave={vi.fn()} onClose={onClose} />,
    );
...
function NotesApp({
  ...
  useEffect(() => {
    window.api.onVoiceProgress((p) =>
      setVoiceDownloadProgress(p >= 1 ? null : p),
    );
  ...
  const handleSaveSettings = useCallback(
    async (hotkeys: typeof notes.settings.hotkeys) => {
