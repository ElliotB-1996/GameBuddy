import type { Profile, Layer, Device, RadialMenu, RadialAction } from '../data/types'
...
function isRewasdFile(json: unknown): json is RewasdFile {
  return (
    ...
    'config' in json && 'mappings' in json &&
...
const DIK_NAMES: Record<string, string> = {
  ...
  DIK_LCONTROL:'Ctrl',  DIK_RCONTROL:'Ctrl',
  DIK_LSHIFT:'Shift',   DIK_RSHIFT:'Shift',
  DIK_LMENU:'Alt',      DIK_RMENU:'Alt',
  DIK_LWIN:'Win',       DIK_RWIN:'Win',
  DIK_ESCAPE:'Esc',     DIK_RETURN:'Enter',  DIK_SPACE:'Space',
  DIK_TAB:'Tab',        DIK_BACK:'Backspace',
  DIK_DELETE:'Del',     DIK_HOME:'Home',     DIK_END:'End',
  DIK_INSERT:'Ins',     DIK_PRIOR:'PgUp',    DIK_NEXT:'PgDn',
  DIK_UP:'↑',           DIK_DOWN:'↓',
  DIK_LEFT:'←',         DIK_RIGHT:'→',
  ...
  DIK_ADD:'+',          DIK_SUBTRACT:'Num-', DIK_MULTIPLY:'Num*',
  DIK_DIVIDE:'Num/',    DIK_DECIMAL:'Num.',
  DIK_NUMLOCK:'NumLock', DIK_NUMPADENTER:'NumEnter',
  DIK_NUMPAD0:'Num0', DIK_NUMPAD1:'Num1', DIK_NUMPAD2:'Num2',
  DIK_NUMPAD3:'Num3', DIK_NUMPAD4:'Num4', DIK_NUMPAD5:'Num5',
  DIK_NUMPAD6:'Num6', DIK_NUMPAD7:'Num7', DIK_NUMPAD8:'Num8', DIK_NUMPAD9:'Num9',
  DIK_CAPITAL:'CapsLock', DIK_SCROLL:'ScrollLock', DIK_PAUSE:'Pause',
  DIK_APPS:'Menu',
}
...
const CYBORG_BTN: Record<number, string> = {
  1:'1',  5:'2',  13:'3', 17:'4',  2:'5',  6:'6',  14:'7',  18:'8',
  3:'9',  7:'10', 15:'11',19:'12', 26:'13', 4:'14',  8:'15', 16:'16',
  20:'17',27:'18',12:'19',11:'20', 10:'22', 9:'23',
  33:'28',35:'29',34:'30',36:'31',
  23:'36',24:'37',25:'38',28:'41',
}

const CYRO_BTN: Record<number, string> = {
  20:'1', 19:'2', 18:'3', 17:'4', 16:'5', 15:'6', 13:'8',
  8:'9',  7:'10', 4:'14', 3:'15', 10:'20', 9:'22',
  33:'28',35:'29',34:'30',36:'31',
}
...
function parseRadialMenus(
  ...
  const virtualMasks = masks.filter(m => m.radialMenuSet && m.radialMenuSet.length > 0)
  ...
  for (const m of file.mappings) {
    if (!m.condition?.shiftId || !radialShiftIds.has(m.condition.shiftId)) continue
  ...
  function buildActions(circleId: number): RadialAction[] {
    ...
    circle.sectors.forEach((sectorId, i) => {
      ...
      if (sector.childCircleId) {
        ...
        actions.push({ label: sector.description ?? 'Submenu', direction, submenu })
        return
      }
  ...
  return triggers.flatMap((trigger, i) => {
    ...
    if (prefixMatch) {
      ...
      if (trigger.description) {
        label = trigger.description.replace(/\s*Trigger\s*$/i, '').trim() || trigger.description
      }
      ...
      if (maskId !== undefined) {
        ...
        if (entry) {
          ...
          triggerBtn = device ? toAzeronId(device, entry.buttonId) : String(entry.buttonId)
        }
      }
    }
...
export function parseRewasd(json: unknown): Profile[] {
  if (!isRewasdFile(json)) throw new ParseError('Not a valid reWASD profile file')
  ...
  for (const mapping of json.mappings) {
    ...
    const layerKey: 'default' | 'shift' = shiftId !== undefined ? 'shift' : 'default'
    const activatorRaw = mapping.condition?.mask?.activator?.type ?? 'single'
    const activator: ActivatorType =
      activatorRaw === 'long'   ? 'long'   :
      activatorRaw === 'double' ? 'double' :
      'single'
    ...
    if (mapping.description) {
      ...
      if (prefixMatch) {
        ...
        const label   = comboMatch ? rest.slice(0, comboMatch.index).trim() : rest.trim()
        setButton(device, layerKey, btnId, label || `Button ${btnId}`, binding, activator)
        continue
      }
    }
    ...
    const binding = mapping.jumpToLayer ? undefined : macrosToBinding(mapping.macros ?? [])
    const label   = mapping.description?.trim() ||
                    (mapping.jumpToLayer ? 'Radial Menu' : (binding ?? `Button ${btnId}`))

    setButton(device, layerKey, btnId, label, binding, activator)
  }

  if (byDevice.size === 0) throw new ParseError('No valid mappings found in reWASD file')
