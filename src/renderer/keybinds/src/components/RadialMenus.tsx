...
function ActionLeaf({ action }: { action: RadialAction }): JSX.Element {
...
function MenuTree({ menu }: { menu: RadialMenu }): JSX.Element {
...
export default function RadialMenus({ menus }: Props): JSX.Element {
  return (
    <div className="panel">
      <div className="panel-title">Radial Menus</div>
      {menus.length === 0 ? (
        <p style={{ fontSize: 11, color: "#6b7280" }}>
          No radial menus defined for this profile.
        </p>
      ) : (
        <ul className="rtree">
          {menus.map((m) => (
            <MenuTree key={m.id} menu={m} />
          ))}
        </ul>
      )}
    </div>
